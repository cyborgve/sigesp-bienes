...
export class DefinicionesBasicasService {
  ...
  public getDefinition(tipo:string){
    return this.http.get(`${this.sigesp.URL}/dao/sbn/definiciones_basicas_dao.php?tipo=${tipo}`,
    {headers: this.sigesp.getHttpHeaders()})
    .pipe(
      map((res:any)=>{
        if(res.success){
          if (res.data != null) {
            res.data=res.data.map(element=> new MDefinicionesBasicas(element));
          } else {
            res.data=[]
          }
      }
  ...
  public saveDefinition(tipo:string, deficiones:MDefinicionesBasicas[]){
    let body=deficiones
    return this.http.post(`${this.sigesp.URL}/dao/sbn/definiciones_basicas_dao.php?tipo=${tipo}`, body, {
      headers: this.sigesp.getHttpHeaders()
    }).pipe(  
       map((res: any) => {
          if (res.success) {
  ...
  public deleteDefinition(delComponetes:MDefinicionesBasicas[], tipo:string){ 
    ...
    return this.http.delete(`${this.sigesp.URL}/dao/sbn/definiciones_basicas_dao.php?ids=${ids}&tipo=${tipo}`, {
    headers: this.sigesp.getHttpHeaders() 
  })
  .pipe(
    map((res:any) => {
