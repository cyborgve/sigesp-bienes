...
export class OrigenService {
  ...
  public getAllOrigin() {
    return this.http
      .get(`${this.sigesp.URL}/dao/sbn/origen_dao.php?`, {
  ...
  public saveOrigin(formGroup: FormGroup, id: number, idProvvedor: number) {
    ...
    return this.http
      .post(`${this.sigesp.URL}/dao/sbn/origen_dao.php`, body, {
        headers: this.sigesp.getHttpHeaders(),
      })
      .pipe(
        map((res: any) => {
  ...
  public updateOrigin(formGroup: FormGroup, id: number, idProvvedor: number) {
    const body = this.getBody(formGroup, id, idProvvedor);
    return this.http
      .put(`${this.sigesp.URL}/dao/sbn/origen_dao.php`, body, {
        headers: this.sigesp.getHttpHeaders(),
      })
      .pipe(
        map((res: any) => {
  ...
  public deleteOrigin(codigo: number) {
    return this.http
      .delete(`${this.sigesp.URL}/dao/sbn/origen_dao.php?codigo=${codigo}`, {
        headers: this.sigesp.getHttpHeaders(),
      })
      .pipe(
        map((res: any) => {
  ...
  public getBody(formGroup: FormGroup, id: number, idProveedor: number) {
    ...
    if (fecfac == null || fecfac == '') {
      fecfac = '1999-01-01';
    } else {
      fecfac = moment(fecfac).format('YYYY-MM-DD');
    }
    ...
    const body = {
      ...
      fecadq: moment(formGroup.get('fechaAdquisicion').value).format(
        'YYYY-MM-DD'
      ),
      modadq: formGroup.get('modoAdquisicion').value,
      fecforadq: moment(formGroup.get('fechaFormaAdquisicion').value).format(
        'YYYY-MM-DD'
      ),
