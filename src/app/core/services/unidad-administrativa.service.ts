...
export class UnidadAdministrativaService {
  ...
  public getAllAdministrativeUnit(){    
    return this.http.get(`${this.sigesp.URL}/dao/sbn/unidad_administrativa_dao.php?`,
    {headers: this.sigesp.getHttpHeaders()})
    .pipe(
      map((res:any) => {
        if (res.success) {
          res.data = res.data.map(element => new MUnidadAdministrativa(element));
        } else {
          res.data = [];
        }
        return res
  ...
  public saveAdministrativeUnit(formGroup: FormGroup){     
    ...
    return this.http.post(`${this.sigesp.URL}/dao/sbn/unidad_administrativa_dao.php`, body, {
      headers: this.sigesp.getHttpHeaders()
    }).pipe(  
       map((res: any) => {
          if (res.success) {
  ...
  public updateAdministrativeUnit(formGroup: FormGroup, id:number){     
    ...
    return this.http.put(`${this.sigesp.URL}/dao/sbn/unidad_administrativa_dao.php`, body, {
      headers: this.sigesp.getHttpHeaders()
    }).pipe(  
       map((res: any) => {
          if (res.success) {
  ...
  public deleteAdministrativeUnit(codigo:number){ 
    return this.http.delete(`${this.sigesp.URL}/dao/sbn/unidad_administrativa_dao.php?codigo=${codigo}`, {
    headers: this.sigesp.getHttpHeaders() 
  })
  .pipe(
    map((res:any) => {
