...
import { MDefinicionesBasicas, DefinicionesSelect } from '@core/models/MDefinicionesBasicas';
...
export class DefinicionesBasicasComponent implements OnInit {
  ...
  public basicDefinitions= new  MatTableDataSource<MDefinicionesBasicas>(this.definiciones);
  ...
  public saveDefinition(){
    this.definicionesBasicasService.saveDefinition( this.seleccionado,this.basicDefinitions.data).subscribe((resp:any)=>{
  ...
  public delDenifinition(){
    ...
    this.delDeficiones=[]   
      if (this.selection.selected.length>0){
        this.sigesp.openDialogConfirm("Eliminar Registro","Esta seguro de eliminar los registro?").then(resp=>{ 
          if (resp){
            for(let i=0; i<this.selection.selected.length; i++){
              let j = this.basicDefinitions.data.findIndex(e => {
                return (e.codigo.trim() == this.selection.selected[i].codigo.trim())
              }) 
              if(j>=0){
        ...
        if(this.delDeficiones.length>0 && seguir){
            this.definicionesBasicasService.deleteDefinition(this.delDeficiones, this.seleccionado).subscribe(resp=>{
                if(resp.data==true) {
  ...
  public selectedDefinition(){
    ...
    this.inicializar()  
    this.definicionesBasicasService.getDefinition(this.seleccionado).subscribe(resp=>{
      this.definiciones=resp.data
      this.basicDefinitions.data=this.definiciones  
    })
  }
  ...
  public validateDuplicates(){
    this.codigoDuplicado=false
     for (let i=0; i<this.basicDefinitions.data.length; i++){
       ...
       let codigoDoble = this.basicDefinitions.data.filter(e => e.codigo == code);          
