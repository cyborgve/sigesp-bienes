...
export class SegurosComponent implements OnInit, OnDestroy {
  ...
  public saveInsurance() {
    this.segurosService
      .saveInsurance(this.formSeguro, this.idSeguro)
      .subscribe((resp: any) => {
        if (resp.success) {
  ...
  public updateInsurance() {
    this.segurosService
      .updateInsurance(this.formSeguro, this.idSeguro)
      .subscribe((resp: any) => {
        if (resp.success) {
          if (resp.data) {
  ...
  public deleteInsurance() {
    this.sigesp
      .openDialogConfirm(
        'Eliminar el Seguro',
        'Esta seguro de eliminar el seguro?'
      )
      .then(resp => {
        if (resp) {
          this.segurosService
            .deleteInsurance(this.idSeguro)
            .subscribe((resp: any) => {
              if (resp.data) {
                this.sigesp.showToastSuccess('Seguro eliminada con éxito');
                this.inicializar();
              } else
                this.sigesp.showToastError('La marca no puede ser eliminada');
            });
        }
      });
  }
      ...
      this.sigesp
        ...
        .then((res: MSeguros) => {
          if (res != null) {
            ...
            this.formSeguro
              .get('denominacion')
              .setValue(res.denominacionSeguro);
            this.formSeguro
              .get('companniaAseguradora')
              .setValue(res.idAseguradora);
