...
export class MarcasModelosComponent implements OnInit {
  ...
  public saveBrand() {
    this.marcaModelService
      .saveBrand(this.formMarca, this.modelsBrand.data)
      .subscribe((resp: any) => {
        if (resp.success) {
  ...
  public updateBrand() {
    if (this.idMarca == 0) {
      ...
      this.marcaModelService
        .updateBrand(this.formMarca, this.idMarca, this.modelsBrand.data)
        .subscribe((resp: any) => {
          if (resp.success) {
            if (resp.data) {
  ...
  public deleteBrand() {
    if (this.idMarca == 0) {
      ...
      this.sigesp
        .openDialogConfirm(
          'Eliminar la marca',
          'Esta seguro de eliminar marca?'
        )
        .then(resp => {
          if (resp) {
            this.marcaModelService
              .deleteBrand(this.idMarca)
              .subscribe((resp: any) => {
                if (resp.data) {
  ...
  public openCatalog() {
    ...
    if (this.allBrand.length > 0) {
      this.sigesp
        .openCatalogoGenerico(columnas, tittle, this.allBrand, nameColummnas)
        .then((resp: MMarcas) => {
          if (resp != null) {
  ...
  public getNexCode() {
    ...
    if (this.modelsBrand.data.length == 0) {
      ...
      this.modelsBrand.data = this.modelsBrand.data.sort((a, b) => {
        if (a.codigoModelo > b.codigoModelo) {
          return 1;
        }
        if (b.codigoModelo > a.codigoModelo) {
          return -1;
        }
        return 0;
      });
  ...
  public delModel() {
    ...
    if (this.selection.selected.length > 0) {
      this.sigesp
        .openDialogConfirm(
          'Eliminar Modelos',
          '¿Esta seguro de eliminar los modelos?'
        )
        .then(resp => {
          if (resp) {
            for (let i = 0; i < this.selection.selected.length; i++) {
              let j = this.modelsBrand.data.findIndex(e => {
                return (
                  e.codigoModelo.trim() ==
                  this.selection.selected[i].codigoModelo.trim()
                );
              });
  ...
  public validateDuplicates() {
    ...
    for (let i = 0; i < this.modelsBrand.data.length; i++) {
      ...
      let codigoDoble = this.modelsBrand.data.filter(
        e => e.codigoModelo == code
      );
