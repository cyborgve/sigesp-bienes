...
export class CausaMovimientoComponent implements OnInit {
  ...
  constructor(
    ...
    private causaMovimientoService: CausaMovimientoService
  ) {
  ...
  public saveCauseMovement() {
    this.causaMovimientoService
      .saveCauseMovement(this.formCausaMovimiento)
      .subscribe((resp: any) => {
        if (resp.data.length > 0) {
          this.sigesp.showToastSuccess(
            'Causa de movimiento guardada con éxito'
          );
          this.inicializar();
  ...
  public updateCauseMovement() {
    this.causaMovimientoService
      .updateCauseMovement(this.formCausaMovimiento, this.idCausa)
      .subscribe((resp: any) => {
        if (resp.data) {
          this.sigesp.showToastSuccess(
            'Causa de movimiento actualizada con éxito'
          );
          this.inicializar();
        } else this.sigesp.showToastError(resp.message);
      });
  ...
  public deleteCauseMovement() {
    this.sigesp
      ...
      .then(resp => {
        if (resp) {
          this.causaMovimientoService
            .deleteCauseMovement(this.idCausa)
            .subscribe((resp: any) => {
              if (resp.data) {
                this.sigesp.showToastSuccess(
                  'Causa de movimiento eliminada con éxito'
                );
                this.inicializar();
  ...
  public openCatalogo() {
    ...
    if (this.AllCause.length > 0) {
      this.sigesp
        .openCatalogoGenerico(columnas, tittle, this.AllCause, nameColummnas)
        .then((resp: MCausaMovimiento) => {
          if (resp != null) {
            this.formCausaMovimiento.get('codigo').setValue(resp.codigoCausa);
            this.formCausaMovimiento
              .get('denominacion')
              .setValue(resp.denominacionCausa);
            this.formCausaMovimiento.get('tipoCausa').setValue(resp.tipoCausa);
            if (resp.estatusAfectacionContable == 1) {
              this.formCausaMovimiento
                .get('estatusContabilidad')
                .setValue(true);
            } else
              this.formCausaMovimiento
                .get('estatusContabilidad')
