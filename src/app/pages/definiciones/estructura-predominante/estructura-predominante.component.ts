...
export class EstructuraPredominanteComponent implements OnInit {
  ...
  public componentesEstructura= new  MatTableDataSource<MComponenteEstructura>(this.componentes);
  ...
  constructor(private router: Router, private sigesp:SigespService,
      ...
      this.formEstructura= new FormGroup({
        denominacion: new FormControl("",[Validators.required, Validators.maxLength(100), Validators.minLength(3)]),
        codigo: new FormControl ("",[Validators.required, Validators.maxLength(4), Validators.minLength(1)]),
      })
  }
  ...
  public getAllEstructure(){
    this.estructuraPredominanteService.getAllPredominantStructure().subscribe(resp=>{
      ...
      if (this.componentesEstructura.data.length>0){
        ...
        if (!this.codigoDuplicado && !this.invalido ){
          ...
          this.sigesp.showToastError('Hay componentes con campos vacios')       
           ...
           this.sigesp.showToastError('Existen codigos de componentes duplicados')
       }
  ...
  public saveStructure(){
    ...
    this.estructuraPredominanteService.savePredominantStructure(this.formEstructura).subscribe((resp:any)=>{
      if (resp.data.length>0){
        this.estructuraPredominanteService.updatePredominantStructure(denominacion,resp.data[0].idTipoEstructura, this.componentesEstructura.data).subscribe(res=>{
          if (res.data){
            this.sigesp.showToastSuccess("Tipo de Estructura guardada con éxito")
          }
        })     
  ...
  public updateStructure(){
    if (this.idEstructura==0){
      this.sigesp.showToastError("La data por defecto no puede ser Modificada")       
          ...
          let denominacion=this.formEstructura.get('denominacion').value;
          this.estructuraPredominanteService.updatePredominantStructure(denominacion, this.idEstructura, this.componentesEstructura.data).subscribe((resp:any)=>{
            if (resp.data){
              this.sigesp.showToastSuccess("Tipo de estructura actualizada con éxito")
              this.inicializar()
            } else  this.sigesp.showToastError(resp.message)       
      })
    }
  ...
  public deleteEstructure(){
    if (this.idEstructura==0){
      ...
      this.sigesp.openDialogConfirm("Eliminar La causa de movimiento","Esta seguro de eliminar la Causa de Movimiento?").then(resp=>{
        if (resp){
          this.estructuraPredominanteService.deletePredominantStructure(this.idEstructura).subscribe((resp: any)=> {
            if (resp.data){
              this.sigesp.showToastSuccess("Tipo de estructura de movimiento eliminada con éxito")
              this.inicializar()
            } else  this.sigesp.showToastError("La estructura no puede ser eliminada")
          })     
        }
      })
    }    
  ...
  public deleteComponentes(){ 
    ...
    if (this.selection.selected.length>0){
      this.sigesp.openDialogConfirm("Eliminar Componentes","Esta seguro de eliminar los componentes?").then(resp=>{ 
        if (resp){
          for(let i=0; i<this.selection.selected.length; i++){
            let j = this.componentes.findIndex(e => {
              return (e.codigoComponenteEstructura.trim() == this.selection.selected[i].codigoComponenteEstructura.trim())
            }) 
            if(j>=0){       
       ...
       if(this.delComponetes.length>0 && seguir){
          this.estructuraPredominanteService.deleteComponet(this.delComponetes).subscribe(resp=>{
            if(resp.data) {
              this.sigesp.showToastSuccess("Componente eliminado con éxito")  
              this.componentesEstructura.data=this.componentes 
            } 
            else {
 ...
 validarDuplicados(){
   this.codigoDuplicado=false
    for (let i=0; i<this.componentesEstructura.data.length; i++){
      ...
      let codigoDoble = this.componentesEstructura.data.filter(e => e.codigoComponenteEstructura == code);          
  ...
  public validarVaciosComponentes(){ 
    let i = this.componentesEstructura.data.findIndex(e=>{
      return (e.codigoComponenteEstructura == "") || (e.denominacionComponenteEstructura=="")
    })         
      if (i >= 1){
         this.invalido=true
  ...
  public openCatalogo (){
   ...
   if (this.allPredominantStructure.length>0){
      this.sigesp.openCatalogoGenerico(columnas,tittle,this.allPredominantStructure,nameColummnas).then((resp:MEstructuraPredominante)=>{
        if(resp !=null) {

          this.formEstructura.get('codigo').setValue(resp.codigoTipoEstructura)
          this.formEstructura.get('denominacion').setValue(resp.denominacionTipoEstructura) 
  ...
  public getComponente(){
   this.estructuraPredominanteService.getStructureComponent(this.idEstructura).subscribe(resp=>{
  ...
  public addComponent(){   
   ...
   public getNexCode()  {
    ...
    if(this.componentesEstructura.data.length==0){
      ...
      this.componentesEstructura.data= this.componentesEstructura.data.sort((a,b)=>{
        if (a.codigoComponenteEstructura>b.codigoComponenteEstructura){return 1}
        if (b.codigoComponenteEstructura>a.codigoComponenteEstructura){return-1}
        return 0
      })
  ...
  SeleccionTodo() {
    ...
    this.componentesEstructura.data.forEach(row => this.selection.select(row));
  }
  ...
  checkboxLabel(row?: MComponenteEstructura): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.codigoComponenteEstructura + 1}`;
  }  
