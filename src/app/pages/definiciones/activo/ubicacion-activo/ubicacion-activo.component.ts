...
@Component({
  selector: 'app-ubicacion-activo',
  templateUrl: './ubicacion-activo.component.html',
  styleUrls: ['./ubicacion-activo.component.scss']
})
export class UbicacionActivoComponent implements OnInit {
  ...
  constructor(private router: Router, 
    ...
    this.formSede= new FormGroup ({
      codigo: new FormControl ("",[Validators.required, Validators.maxLength(2), Validators.minLength(1)]),
      denominacion: new FormControl("",[Validators.required, Validators.maxLength(180), Validators.minLength(3)]),
      ...
      urbanizacion: new FormControl("",[ Validators.maxLength(60), Validators.minLength(3)]),
      calle: new FormControl("",[ Validators.maxLength(60), Validators.minLength(3)]),
      casa: new FormControl("",[ Validators.maxLength(60), Validators.minLength(1)]),
      piso: new FormControl("",[ Validators.maxLength(6), Validators.minLength(1)]),
    })
}
...
public getStates(event: any){   
  ...
  this.cities= this.cities.filter((e)=>(e.codest==event.value && e.codpai==this.codPais))
  this.municipality=this.municipality.filter((e)=>e.countryCode == event.value) 
...
public updateSeat(){  
  this.sedesService.updateSeat(this.formSede, this.idSeat).subscribe((resp:any)=>{
    if (resp.data){
...
public deleteSeat(){ 
  this.sigesp.openDialogConfirm("Eliminar Sede","Esta seguro de eliminar la Sede?").then(resp=>{
    if (resp){
...
public openCatalogo(){  
 ...
 if (this.allSeat.length>0){
    this.sigesp.openCatalogoGenerico(columnas,tittle,this.allSeat,nameColummnas).then((resp:MSede)=>{
      if(resp !=null) {
