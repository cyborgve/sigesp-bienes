...
export class OrigenComponent implements OnInit {
  ...
  public openCatProveedores() {
    ...
    this.sigesp.getProvidersAndBeneficiaries().subscribe(resp => {
      ...
      this.sigesp
        .openCatalogoGenerico(columnas, tittle, this.provider, nameColummnas)
        .then((resp: MProviderBeneficiary) => {
  ...
  public acquisitionform(event: any) {
    this.formaAdquisicion = this.FormaAdquisicion.filter(
      e => e.tipo == parseInt(event.value)
    );
  }
  ...
  public saveOrige() {
    this.origenService
      .saveOrigin(this.formOrigen, this.idOrigin, this.idProveedor)
      .subscribe((resp: any) => {
  ...
  public updateOrigen() {
    this.origenService
      .updateOrigin(this.formOrigen, this.idOrigin, this.idProveedor)
      .subscribe((resp: any) => {
        if (resp.data) {
  ...
  public openCatalogo() {
    ...
    if (this.allOrigin.length > 0) {
      this.sigesp
        .openCatalogoGenerico(columnas, tittle, this.allOrigin, nameColummnas)
        .then((resp: MOrigen) => {
          if (resp != null) {
            console.log(resp);
            ...
            this.formOrigen
              ...
              .setValue(resp.fechaAdquisicion);
            this.formOrigen
              .get('modoAdquisicion')
              .setValue(resp.modoAquisicion);
            this.formOrigen
              .get('formaAdquisicion')
              .setValue(resp.formaAdquisicion);
            this.formOrigen
              .get('fechaFormaAdquisicion')
              .setValue(resp.fechaFormaAdquisicion);
            this.formOrigen
              .get('numeroFormaAdquisicion')
              .setValue(resp.numeroFormaAdquisicion);
            this.formOrigen
              .get('nombreFormaAdquisicion')
              .setValue(resp.nombreFormaAdquisicion);
            ...
            this.formOrigen
              .get('nombrePropietarioAnterior')
              .setValue(resp.nomprePropietarioAnterior);
            this.formOrigen
              .get('beneficiarioCedente')
              .setValue(resp.nombreBeneficiarioCedente);
            this.formOrigen
              .get('beneficiarioReceptor')
              .setValue(resp.nombreBeneficiarioReceptor);
