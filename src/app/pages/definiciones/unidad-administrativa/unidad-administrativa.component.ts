...
export class UnidadAdministrativaComponent implements OnInit {
  ...
  constructor(private router:Router,
    ...
    this.formUnidadAdministrativa=new FormGroup({
      codigo:new FormControl("", [Validators.required, Validators.maxLength(5), Validators.minLength(1)]),
      denominacion: new FormControl("",[Validators.required, Validators.maxLength(200), Validators.minLength(3)]),
      categoria: new FormControl(),
      otraDenominacion: new FormControl("",[Validators.maxLength(200), Validators.minLength(3)]),
  ...
  public getAllAdministrativeUnit(){
    this.unidadAdministrativaService.getAllAdministrativeUnit().subscribe(resp=>{
  ...
  public saveUnit(){
    this.unidadAdministrativaService.saveAdministrativeUnit(this.formUnidadAdministrativa).subscribe((resp:any)=>{
      if (resp.data.length>0){
        this.sigesp.showToastSuccess("Unidad Administrativa guardada con éxito")
        this.inicializar()
  ...
  public updateUnit(){
    this.unidadAdministrativaService.updateAdministrativeUnit(this.formUnidadAdministrativa, this.idUnit).subscribe((resp:any)=>{
      if (resp.data){
        this.sigesp.showToastSuccess("Unidad Administrativa actualizada con éxito")
        this.inicializar()
      } else  this.sigesp.showToastError(resp.message)
  ...
  public deleteUnit(){
    this.sigesp.openDialogConfirm("Eliminar La causa de movimiento","Esta seguro de eliminar la Causa de Movimiento?").then(resp=>{
      if (resp){
        this.unidadAdministrativaService.deleteAdministrativeUnit(this.idUnit).subscribe((resp: any)=> {
          if (resp.data){
            this.sigesp.showToastSuccess("Unidad Administrativa eliminada con éxito")
            this.inicializar()
  ...
  public openCatalogo (){
    ...
    let columnas = ['codigoUnidadAdministrativa','denominacionUnidadAdministrativaBien']
   if (this.allUnit.length>0){
      this.sigesp.openCatalogoGenerico(columnas,tittle,this.allUnit,nameColummnas).then((resp:MUnidadAdministrativa)=>{
        if(resp !=null) {
          this.formUnidadAdministrativa.get('codigo').setValue(resp.codigoUnidadAdministrativa)
          this.formUnidadAdministrativa.get('denominacion').setValue(resp.denominacionUnidadAdministrativaBien)  
          this.formUnidadAdministrativa.get('categoria').setValue(resp.idCategoriaUnidad)
          this.formUnidadAdministrativa.get('otraDenominacion').setValue(resp.denominacion)
          this.formUnidadAdministrativa.get('codUnidadAdscrita').setValue(resp.idUnidadAdministrativa)
